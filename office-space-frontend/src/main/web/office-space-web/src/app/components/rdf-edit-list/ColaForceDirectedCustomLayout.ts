import { ClusterNode, Edge, Graph, Layout, Node } from '@swimlane/ngx-graph/lib/models';
import * as d3Dispatch from 'd3-dispatch';
import * as d3Force from 'd3-force';
import * as d3Timer from 'd3-timer';
import { Observable, Subject } from 'rxjs';
import { d3adaptor, Group, ID3StyleLayoutAdaptor, InputNode, Layout as ColaLayout, Link, Rectangle } from 'webcola';
import { id } from './id';
                ...
                ...this.inputGraph.clusters.map(
                    (cluster): Group => ({
                        ...
                        groups: cluster.childNodeIds
                            .map(nodeId => this.inputGraph.clusters.findIndex(node => node.id === nodeId) as any)
                            .filter(x => x >= 0),
                        leaves: cluster.childNodeIds
                            .map(nodeId => this.inputGraph.nodes.findIndex(node => node.id === nodeId) as any)
                            .filter(x => x >= 0)
                    })
                )
            ],
